// Web Audio API Synthesizer for party game sound effects (no external audio assets required)
...
export function playVoteSound(enabled: boolean = true) {
  if (!enabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
export function playSecretRevealSound(isImpostor: boolean, enabled: boolean = true) {
  if (!enabled) return;
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    ...
    if (isImpostor) {
      ...
      osc.frequency.exponentialRampToValueAtTime(164.81, ctx.currentTime + 0.35); // E3
